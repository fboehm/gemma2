...
#' @param V_e error covariance matrix
#' @export
MphEM2 <- function(max_iter = 10000, max_prec = 1 / 1000, eval, X1, X2, Y, V_g = diag(1, 2), V_e = diag(1, 2)){
  n_size <- length(eval)
  # be careful with defining c_size here
  ...
  #if (func_name == "R"){
  logl_const <- - (n_size - c_size) * d_size * log(2 * pi) + d_size * log(det(XXt)) / 2
  #}
  out <- list()
  #logl_old <- 1 # we need to define logl_old before using it within the EM iterations
  # start EM
  for (t in 1:max_iter){
    ...
    UltVehiY <- UltVehi %*% Y
    #
    xHiy1 <- calc_XHiY(eval, D_l, X1, UltVehiY)
